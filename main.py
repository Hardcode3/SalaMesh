...
A = scipy.sparse.lil_matrix((m.ncorners+nhor+ nb_fault_opposite , m.nverts))
b = [0] * A.shape[0]
...
for c in range(m.ncorners): # lift all vertices of all horizons
    if attr.horizon_id[c]>=0:
        height = (1+attr.horizon_id[c])/200. # arbitrarily chosen coeff to get a visually nice result
...
m.save("evolution/13_chevron.obj")
